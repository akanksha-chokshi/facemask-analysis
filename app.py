...
if types:
    ...
    st.write("Rating values are quite close to each other in general, but we can see that disposable N95 masks are rated more favourably, followed by disposable regulars and then reusable.")
...
if reviews:
    ...
    st.caption("Despite adjusting the threshold for positive reviews, we can see that reviews largely tend towards positive in general, followed by neutral (which the model would classify as positive anyway).")
    st.caption("Perhaps this is something about the iHerb platform - it is usually satisfied customers who review products.")
    st.caption("Or perhaps it is simply the general nature of the product - people purchase the types of masks that might exactly what they are looking for. Other than comfort, they usually would not complain about factors like cost post-purchase.")
    ...
    st.altair_chart(c)
    st.caption("We can observe something interesting - while the two metrics cluster relatively close to each other, the difference over thousands of reviews might still be significant.")
    st.caption("Reusable masks have the lowest rating value but the highest sentiment score - which makes sense if you think about it. Reusable masks are often bought for comfort, and it is likely that if consumers were to be dissatisfied, it would be after purchase, thereby translating into lower rating values. However, if consumers were to be extremely satisfied with their purchase, it would also be in terms of comfort (a benefit they can evaluate and review more expressively, compared to health/safety or cost).")
    st.caption("In both cases, N95 masks beform better than Regular masks in the Disposable category in terms of ratings and reviews. It could also be that these masks have higher brand loyalty and are thus reviewed more, while consumers who buy masks for price and convenience might not be going back to review them, unless extremely dissatisfied.")
    with st.expander ("View Data"):
    ...
    st.altair_chart(c)
    st.caption("When we look at these two metrics through personas, a clearer hierarchy emerges.")
    st.caption("We can see that consumers that prefer health/safety rate and review masks the highest, followed by the general-comfort consumers. Low-cost-seeking consumers tend to value the product the worst, followed by the environment-friendly outlier, which was actually really badly reviewed.")
    st.caption("When we split the disposable N95s into two categories: ones that emphasise health/safety and ones that emphasise cost and convenience, the former jumps up the ratings above the reusable/comfort category, while pushing the low-cost disposable ones further below.")
    with st.expander ("View Data"):
    ...
    st.write("Here, we can see that general/comfort products are more represented in this list, followed by health-focused products. This shows that health-focused products are more consistently reviewed, while comfort-general-focused ones have significant variation, with some being the top rated/reviews products and some pulling the average down to below health-focused products.")
    ...
    st.write("We can see that a lot of keywords relate to comfort - 'breathe', 'fit', 'fabric', 'size' - which shows that comfort might be one of the most important factors for general consumers as well.")
...
if conclusion:
    ...
    st.success("I would recommend looking into reusable N95s, sold in a pack of around 1-3 (same as premium comfort masks) and priced around 5-10 AUD per mask (rough estimate given existing prices).")
    ...
    st.write("Currently, it would be interesting to look at a more general ecommerce platform (not specialised like iHerb) and check out their current listings of reusable N95s. Amazon listing data is easy to scrape and there are some tools available for that. ")
